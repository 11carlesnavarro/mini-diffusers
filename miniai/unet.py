"""
A U-Net model for image generation.
"""
...
class ResBlock(TimestepBlock):
    ...
    :param emb_channels: the number of timestep ebedding channels.
    :param dropout: the rate of dropout.
    :param out_channels: if specified, the number of output channels.
    :param use_conv: if True and out channels is specified, use a spatial convolution
                        instead of a 1x1 convolution to change the channels in the skip connection.
    ...
    def forward(self, x, emb):
        """
        Apply the block to a Tensor, conditioned on timestep embeddings.

        :param x: an [N x C x ...] Tensor.
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv, encoder_kv=None):
        ...
        if encoder_kv is not None:
            ...
            k = torch.cat((k, ek), dim=-1)
            v = torch.cat((v, ev), dim=-1)
        ...
        weight = torch.softmax(weight.float(), dim=-1).type_as(weight)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if num_heads_channels == -1:
            ...
            ), f"q,k,v channels {channels} must be divisible by num_heads_channels {num_heads_channels}"
            self.num_heads = channels // num_heads_channels
        self.use_checkpoint = use_checkpoint
        self.norm = normalization(channels)
        self.qkv = conv_nd(dims, channels, 3 * channels, 1)
        self.attention_type = attention_type
        if attention_type == "flash":
            raise NotImplementedError("Flash attention not yet implemented.")
        else:
        ...
        if encoder_channels is not None:
            ...
            self.encoder_kv = conv_nd(1, encoder_channels, 2 * channels, 1)
...
class UNetModel(nn.Module):
    """
    The full U-Net model with attention and timestep embeddings.
    
    :param in_channels: channels in the input Tensor.use_attention_checkpoint
    ...
    def forward(self, inp): #x, timesteps, y=None):
        ...
        :param x: an [N x C x ...] input tensor.
